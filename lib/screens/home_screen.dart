...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  static const int _serviceReminderKmThreshold = 5; // Notifikasi jika kurang dari 5 KM
  ...
  void _setupNotificationClickListener() {
    _notificationPayloadSubscription = notificationPayloadStream.stream.listen((payload) {
      ...
      if (payload == "confirm_trip_via_stop_notification") {
        if (_pendingTripConfirmationData != null) {
          print("[HomeScreen] Menampilkan dialog konfirmasi perjalanan untuk data pending.");
          _showTripConfirmationDialog(_pendingTripConfirmationData!);
        } else {
          print("[HomeScreen] Payload 'confirm_trip_via_stop_notification' diterima, tapi tidak ada data perjalanan pending.");
        }
      } else if (payload != null && payload.startsWith("service_reminder_odo_")) {
        print("[HomeScreen] Notifikasi servis di-tap, payload: $payload. Navigasi ke Jadwal.");
  ...
  Future<void> _requestNotificationPermissionIfNeeded() async {
    if (Platform.isAndroid) {
      ...
      final bool? currentPermissionStatus = await androidImplementation?.areNotificationsEnabled();
      print("[HomeScreen] Status Izin Notifikasi Android saat ini: $currentPermissionStatus");
      ...
      if (currentPermissionStatus != null && !currentPermissionStatus) {
          ...
          if (permissionGranted != null && permissionGranted) {
            print("[HomeScreen] Izin notifikasi DIBERIKAN oleh pengguna setelah diminta.");
          } else {
            print("[HomeScreen] Izin notifikasi DITOLAK oleh pengguna setelah diminta.");
          }
      } else if (currentPermissionStatus == null) {
        print("[HomeScreen] Tidak dapat memeriksa status izin notifikasi (mungkin platform < Android 13).");
      } else {
  ...
  Future<void> _loadInitialDashboardData({bool checkServiceNotif = true}) async {
    if (!mounted) return;
    setState(() {
      _isLoading = true;
      _upcomingSchedules = [];
      ...
      _liveCoordinatesDisplay = _currentVehicleIdForTracking != null ? "Memulai pelacakan..." : "Pelacakan tidak aktif.";
    });

    _userNameForAppBar = prefs.getString(UserService.prefUserName) ?? "Pengguna";
    String? userEmail = prefs.getString(UserService.prefUserEmail);
    String? userPhotoUrlFromPrefs = prefs.getString(UserService.prefUserPhotoUrl);
    ...
    if (vehicleList.isNotEmpty) {
        ...
        await prefs.setString(UserService.prefCurrentVehicleId, _currentVehicleIdForTracking!);
    } else {
     ...
     if (_currentVehicleIdForTracking != null && fetchedPrimaryVehicle != null) {
      try {
        _upcomingSchedules = await _vehicleService.getSchedules(_currentVehicleIdForTracking!);
        _serviceHistoryForEstimates = await _vehicleService.getServiceHistory(_currentVehicleIdForTracking!);
        print("[HomeScreen] Jadwal (${_upcomingSchedules.length}) & Riwayat servis (${_serviceHistoryForEstimates.length}) dimuat untuk kendaraan ID: $_currentVehicleIdForTracking");
      } catch (e) {
        print("[HomeScreen] Error mengambil jadwal atau riwayat untuk kendaraan ID $_currentVehicleIdForTracking: $e");
      }
    }
    ...
    if (mounted) {
      setState(() {
        ...
        if (_primaryVehicle != null) {
            ...
            _liveOdometerDisplay = "${NumberFormat.decimalPattern('id_ID').format(_primaryVehicle!.currentOdometer)} km";
            _appBarTitle = _primaryVehicle!.model;
            if (checkServiceNotif) {
              print("[HomeScreen] Memanggil _checkUpcomingServicesAndNotify dari _loadInitialDashboardData.");
    ...
    print("[HomeScreen] Status pelacakan saat ini: $isCurrentlyTracking. ID Kendaraan untuk dilacak: $_currentVehicleIdForTracking");

    if (_currentVehicleIdForTracking != null && !isCurrentlyTracking) {
      print("[HomeScreen] Memulai pelacakan untuk kendaraan: $_currentVehicleIdForTracking");
      ...
      if (mounted) setState(() { _liveCoordinatesDisplay = "Pelacakan dimulai..."; });
    } else if (_currentVehicleIdForTracking == null && isCurrentlyTracking) {
        _locationService.stopTracking();
        print("[HomeScreen] Pelacakan dihentikan karena tidak ada kendaraan yang aktif.");
        if (mounted) setState(() { _liveCoordinatesDisplay = "Pelacakan dihentikan."; });
    } else if (_currentVehicleIdForTracking != null && isCurrentlyTracking) {
      print("[HomeScreen] Pelacakan sudah aktif untuk kendaraan: $_currentVehicleIdForTracking");
      if (mounted) setState(() { _liveCoordinatesDisplay = "Pelacakan aktif..."; });
    } else {
       print("[HomeScreen] Pelacakan tidak dimulai: Tidak ada kendaraan aktif atau pelacakan sudah berjalan dengan kondisi sesuai.");
       if (mounted) setState(() { _liveCoordinatesDisplay = "Pelacakan tidak aktif."; });
    }
  ...
  void _handleLiveLocationUpdate(loc.LocationData currentLocation, double accumulatedSegmentDistanceKm) {
    if (!mounted) return;
    double totalLiveOdometerDouble = _odometerSnapshotAtTrackingStart.toDouble() + accumulatedSegmentDistanceKm;
    NumberFormat odometerFormatter = NumberFormat("#,##0.0", "id_ID");
    if (accumulatedSegmentDistanceKm == 0.0 && totalLiveOdometerDouble == _odometerSnapshotAtTrackingStart.toDouble()) {
        _liveOdometerDisplay = "${NumberFormat.decimalPattern('id_ID').format(_odometerSnapshotAtTrackingStart)} km";
    } else {
        _liveOdometerDisplay = "${odometerFormatter.format(totalLiveOdometerDouble)} km";
    }

    if (currentLocation.latitude != null && currentLocation.longitude != null) {
      _liveCoordinatesDisplay = "Lat: ${currentLocation.latitude!.toStringAsFixed(5)}, Lon: ${currentLocation.longitude!.toStringAsFixed(5)}";
    } else {
  ...
  void _handleMotorStoppedWithData(TentativeTripData tentativeData) {
    print("[HomeScreen] _handleMotorStoppedWithData TERPANGGIL. Jarak tentatif: ${tentativeData.distanceKm} km");
    
    if (!mounted) {
      print("[HomeScreen] _handleMotorStoppedWithData: Widget tidak mounted, abaikan.");
      return;
    }
    // Hanya proses jika tidak ada konfirmasi yang sudah pending
    if (_pendingTripConfirmationData != null) {
      print("[HomeScreen] _handleMotorStoppedWithData: Sudah ada data perjalanan pending, abaikan trigger baru.");
      return;
    }
    ...
    if (tentativeData.endLocation.latitude != null && tentativeData.endLocation.longitude != null) {
      locationInfo = "di sekitar Lat: ${tentativeData.endLocation.latitude!.toStringAsFixed(3)}, Lon: ${tentativeData.endLocation.longitude!.toStringAsFixed(3)}.";
    }

    print("[HomeScreen] MEMICU NOTIFIKASI MOTOR BERHENTI (15 detik)... Payload: confirm_trip_via_stop_notification");
  ...
  Future<void> _showTripConfirmationDialog(TentativeTripData tripData) async {
    ...
    if (currentPendingData == null || _primaryVehicle == null) {
      print("[HomeScreen] Tidak bisa menampilkan dialog konfirmasi: data pending null atau primaryVehicle null.");
    ...
    bool? useMotor = await showDialog<bool>(
      ...
      builder: (BuildContext alertContext) => AlertDialog(
        ...
        content: Text('Perjalanan sekitar ${currentPendingData.distanceKm.toStringAsFixed(2)} km terdeteksi berakhir. Apakah Anda menggunakan motor ${_primaryVehicle!.plateNumber} untuk perjalanan ini?'),
        actions: <Widget>[
          TextButton(
    ...
    if (useMotor == true) {
      ...
      final Map<String, dynamic> tripPayload = {
        ...
        if (currentPendingData.startLocation.latitude != null) 'start_latitude': currentPendingData.startLocation.latitude,
        if (currentPendingData.startLocation.longitude != null) 'start_longitude': currentPendingData.startLocation.longitude,
        if (currentPendingData.endLocation.latitude != null) 'end_latitude': currentPendingData.endLocation.latitude,
        if (currentPendingData.endLocation.longitude != null) 'end_longitude': currentPendingData.endLocation.longitude,
      };
      final result = await _vehicleService.addTrip(_primaryVehicle!.vehicleId.toString(), tripPayload);

      if (mounted) {
        if (result['success']) {
          final newOdometerRaw = result['data']?['newOdometer'];
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Perjalanan ${currentPendingData.distanceKm.toStringAsFixed(2)} km berhasil dicatat!')),
          );
          if (newOdometerRaw != null) {
            setState(() {
              int updatedOdometer;
              if (newOdometerRaw is int) { updatedOdometer = newOdometerRaw; }
              else if (newOdometerRaw is String) { updatedOdometer = int.tryParse(newOdometerRaw) ?? _primaryVehicle!.currentOdometer; }
              else if (newOdometerRaw is double) { updatedOdometer = newOdometerRaw.round(); }
              else { updatedOdometer = _primaryVehicle!.currentOdometer; }
              _primaryVehicle!.currentOdometer = updatedOdometer;
              _odometerSnapshotAtTrackingStart = updatedOdometer; // Penting untuk update snapshot
              _liveOdometerDisplay = "${NumberFormat.decimalPattern('id_ID').format(updatedOdometer)} km";
            });
            print("[HomeScreen] Odometer diperbarui ke: ${_primaryVehicle!.currentOdometer}. Memeriksa notifikasi servis.");
            _checkUpcomingServicesAndNotify(isInitialCheck: false); // Cek setelah perjalanan dicatat
          }
      ...
      print("[HomeScreen] Pengguna KONFIRMASI TIDAK atau menutup dialog untuk perjalanan.");
    }
  ...
  Future<void> _checkUpcomingServicesAndNotify({required bool isInitialCheck}) async {
    ...
    print("[HomeScreen] Memeriksa jadwal servis untuk notifikasi. InitialCheck: $isInitialCheck. Odo Saat Ini: ${_primaryVehicle!.currentOdometer}");
    ...
    for (var schedule in _upcomingSchedules) {
      if (schedule.nextDueOdometer != null &&
          (schedule.status?.toUpperCase() == "UPCOMING" ||
           ...
           schedule.status == null)) { // Anggap null sebagai pending/upcoming
        ...
        String prefKeyReminder = 'notified_reminder_service_${scheduleId}_odo_${targetOdo}';
        String prefKeyOverdue = 'notified_overdue_service_${scheduleId}_odo_${targetOdo}';

        if (kmRemaining > 0 && kmRemaining <= _serviceReminderKmThreshold) {
          bool alreadyNotifiedReminder = prefs.getBool(prefKeyReminder) ?? false;
          if (!alreadyNotifiedReminder || isInitialCheck) {
            print("[HomeScreen] SERVIS MENDEKAT: $serviceName tinggal $kmRemaining km lagi. Mengirim notifikasi...");
            await _notificationService.showLocalNotification(
              id: 3000 + scheduleId, // ID unik
              title: "Pengingat Servis Segera",
              body: "Waktunya servis $serviceName! Tinggal $kmRemaining km lagi (Target: $targetOdo km).",
              payload: "service_reminder_odo_${scheduleId}"
            );
            await prefs.setBool(prefKeyReminder, true);
            await prefs.remove(prefKeyOverdue); // Reset flag overdue jika sudah tidak overdue
          } else {
            // print("[HomeScreen] Servis $serviceName ($kmRemaining km lagi) sudah pernah dinotifikasi (reminder).");
          }
        } else if (kmRemaining <= 0) { // Sudah terlewat atau pas
          bool alreadyNotifiedOverdue = prefs.getBool(prefKeyOverdue) ?? false;
          if (!alreadyNotifiedOverdue || isInitialCheck) {
            print("[HomeScreen] SERVIS TERLEWAT (ODO): $serviceName. Odo saat ini: $currentOdo, Target: $targetOdo. Mengirim notifikasi...");
            await _notificationService.showLocalNotification(
              id: 4000 + scheduleId, // ID unik berbeda
              title: "PERHATIAN: Servis Terlewat!",
              body: "Servis $serviceName sudah melewati batas odometer (Target: $targetOdo km). Segera lakukan servis!",
              payload: "service_overdue_odo_${scheduleId}"
            );
            await prefs.setBool(prefKeyOverdue, true);
            await prefs.remove(prefKeyReminder); // Reset flag reminder jika sudah overdue
          } else {
  ...
  Future<void> _showManualOdometerUpdateDialog() async {
          ...
          return AlertDialog(
            ...
            content: Form(
                ...
                child: TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: 'Odometer Baru (km)', hintText: 'Masukkan angka odometer'),
                  validator: (value) {
                    if (value == null || value.isEmpty) { return 'Odometer tidak boleh kosong'; }
                    final int? newOdo = int.tryParse(value);
                    if (newOdo == null) { return 'Masukkan angka yang valid'; }
                    if (newOdo <= (_primaryVehicle?.currentOdometer ?? 0)) { return 'Odometer baru harus lebih besar dari saat ini (${_primaryVehicle?.currentOdometer ?? 0} km)';}
                    return null;
                  },
                )
            ),
            actions: <Widget>[
              TextButton(child: const Text('Batal'), onPressed: () => Navigator.of(dialogContext).pop(null)),
    ...
    if (newOdometerValue != null) {
      final result = await _vehicleService.updateOdometerManually(_currentVehicleIdForTracking!, newOdometerValue);
      if (mounted) {
        if (result['success']) {
           ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Odometer berhasil diperbarui menjadi $newOdometerValue km.')));
          setState(() {
            _primaryVehicle!.currentOdometer = newOdometerValue;
            _odometerSnapshotAtTrackingStart = newOdometerValue;
            _liveOdometerDisplay = "${NumberFormat.decimalPattern('id_ID').format(newOdometerValue)} km";
            _displayData = UserData.combine( {'name': _userNameForAppBar, 'email': prefs.getString(UserService.prefUserEmail), 'userPhotoUrl': prefs.getString(UserService.prefUserPhotoUrl)}, _primaryVehicle);
          });
           // Muat ulang data dashboard, termasuk jadwal, lalu cek notifikasi servis
           await _loadInitialDashboardData(checkServiceNotif: true);
        } else {
           ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(result['message'] ?? 'Gagal update odometer.')));
        }
      }
    }
  }
  ...
  Future<void> _loadRecentTrips() async {
    ...
    try {
      _recentTrips = await _vehicleService.getRecentTrips(
        _primaryVehicle!.vehicleId.toString(), limit: 3);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Gagal memuat riwayat perjalanan: ${e.toString()}')),
        );
      }
    } finally {
  ...
  ScheduleItem? _getScheduleByType(String serviceType) {
    try {
      return _upcomingSchedules.firstWhere(
          (s) => s.itemName.toLowerCase() == serviceType.toLowerCase() &&
                 (s.status?.toUpperCase() != 'COMPLETED' && s.status?.toUpperCase() != 'SKIPPED')
      );
    } catch (e) {
      return null;
    }
  ...
  DateTime? _getLastOilServiceDate() {
    try {
      final oilServices = _serviceHistoryForEstimates
          .where((h) => h.serviceType.toLowerCase() == _oliMesinServiceType.toLowerCase())
          .toList();
    ...
    if (_primaryVehicle != null && _primaryVehicle!.lastServiceDate != null && _primaryVehicle!.lastServiceDate!.isNotEmpty) {
  ...
  int _getLastOilServiceOdometer() {
    try {
      final oilServices = _serviceHistoryForEstimates
          ...
          .toList();
      if (oilServices.isNotEmpty) {
        oilServices.sort((a, b) => b.odometerAtService.compareTo(a.odometerAtService));
  ...
  Widget build(BuildContext context) {
    ...
    if (_primaryVehicle != null) {
        ...
        if(lastActualOilServiceDate != null){
            lastOilServiceDateFormatted = DateFormat('dd MMM yy', 'id_ID').format(lastActualOilServiceDate);
        } else {
        ...
        if (oliSchedule != null && oliSchedule.nextDueOdometer != null) {
            ...
            nextOliServiceInfo = "Target berikutnya di ${NumberFormat.decimalPattern('id_ID').format(targetOdoForDisplay)} km";
        } else {
            ...
            nextOliServiceInfo = "Target berikutnya ~${NumberFormat.decimalPattern('id_ID').format(targetOdoForDisplay)} km (Est)";
            kmEstSuffix = " (Est)";
        }
        ...
        kmRemainingForOli = targetOdoForDisplay > _primaryVehicle!.currentOdometer
                            ...
                            : 0;
        kmRemainingOliText = "${NumberFormat.decimalPattern('id_ID').format(kmRemainingForOli)} Km Lagi$kmEstSuffix";
        ...
        if (oliSchedule != null) {
            if (oliSchedule.status?.toUpperCase() == "OVERDUE") {
                ...
                final DateTime dueDate = DateTime.parse(formatter.format(oliSchedule.nextDueDate!));
                ...
                if (lastActualOilServiceDate != null) {
                    DateTime estimatedNextDate = DateTime(lastActualOilServiceDate.year, lastActualOilServiceDate.month + _defaultOilChangeIntervalMonths, lastActualOilServiceDate.day);
                    final now = DateTime.now();
                    final DateFormat formatter = DateFormat('yyyy-MM-dd');
                    final DateTime today = DateTime.parse(formatter.format(now));
                    final DateTime estNextDateNormalized = DateTime.parse(formatter.format(estimatedNextDate));
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.notifications_none_outlined), onPressed: () => Navigator.pushNamed(context, NotificationListScreen.routeName)),
          IconButton(icon: const Icon(Icons.logout_outlined), onPressed: _handleLogout),
        ],
      ),
              ...
              ? Center(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.no_transfer_outlined, size: 60, color: Colors.grey.shade400),
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 20),
                         if (_locationService.isTrackingActive() && _currentVehicleIdForTracking != null)
                            Padding(
                                ...
                                child: Row(
                                    ...
                                    children: [
                                        ...
                                        Text("Pelacakan perjalanan aktif", style: TextStyle(color: Colors.green.shade700, fontStyle: FontStyle.italic, fontSize: 12)),
                                    ],
                                ),
                            )
                        else
                            Padding(
                                ...
                                child: Row(
                                    mainAxisAlignment: MainAxisAlignment.center,
                                    children: [
                                        Icon(Icons.location_off_outlined, color: Colors.red.shade700, size: 16),
                                        const SizedBox(width: 6),
                                        Text("Pelacakan perjalanan tidak aktif", style: TextStyle(color: Colors.red.shade700, fontStyle: FontStyle.italic, fontSize: 12)),
                                    ],
                                ),
                            ),
                      ],
  ...
  Widget _buildProfileHeader() {
    ...
    if (vehicleLogoPath != null && vehicleLogoPath.isNotEmpty) {
        String finalLogoUrl = vehicleLogoPath.startsWith('http') ? vehicleLogoPath : _baseImageUrl + vehicleLogoPath;
        vehicleLogoWidget = Padding(
            ...
            child: Image.network(
              ...
              errorBuilder: (context, error, stackTrace) {
                return Icon(Icons.motorcycle_outlined, size: 45, color: Colors.grey.shade500);
              },
            ),
          );
        ...
        vehicleLogoWidget = Padding(
              padding: const EdgeInsets.only(left: 12.0),
              child: Icon(Icons.motorcycle_outlined, size: 45, color: Colors.grey.shade500)
        );
    }
            ...
            children: [
              ...
              Text(
                (vehicleBrand.isNotEmpty && vehicleModelString.isNotEmpty)
                    ...
                    : (vehicleModelString.isNotEmpty ? vehicleModelString : vehicleBrand),
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              crossAxisAlignment: CrossAxisAlignment.center,
              children: [
                const Text("Odometer Saat Ini", style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Colors.blueGrey)),
                TextButton.icon(
                  icon: Icon(Icons.edit_outlined, size: 16, color: Theme.of(context).primaryColor),
                  label: Text("Update", style: TextStyle(fontSize: 12, color: Theme.of(context).primaryColor)),
  ...
  Widget _buildServiceInfoCard({
    ...
    if (status != null) {
        if (status.toUpperCase() == "OVERDUE") {
            cardColor = Colors.red.shade50; statusTextColor = Colors.red.shade700;
            statusIconData = Icons.error_outline_rounded; statusIconColor = Colors.red.shade700;
        } else if (status.toUpperCase() == "UPCOMING") {
            cardColor = Colors.orange.shade50; statusTextColor = Colors.orange.shade700;
            statusIconData = Icons.notification_important_outlined; statusIconColor = Colors.orange.shade700;
        } else if (status.toUpperCase() == "PENDING") {
            cardColor = Colors.blue.shade50; statusTextColor = Colors.blue.shade700;
            statusIconData = Icons.hourglass_empty_outlined; statusIconColor = Colors.blue.shade700;
        }
    }
    return Card(
      color: cardColor,
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Row(children: [ Icon(icon, size: 26, color: Theme.of(context).primaryColor), const SizedBox(width: 10), Text(title, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.bold))]),
            const SizedBox(height: 8),
            Text("Servis Oli Terakhir: $lastServiceDate", style: TextStyle(color: Colors.grey[600], fontSize: 11)),
            const Divider(height: 16, thickness: 0.8),
            Text(nextServiceInfo, style: const TextStyle(fontWeight: FontWeight.w500, fontSize: 13)),
            const SizedBox(height: 8),
            Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                    Row(
                        children: [
                            ...
                            Text(daysRemaining, style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: statusTextColor)),
                        ],
                    ),
                    Text(kmRemaining, style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.green.shade600))
            ]),
          ],
        ),
      ),
        ...
        if (_isLoadingTrips)
          ...
          Card(
            child: const ListTile(
              ...
              title: Text("Belum terdapat data perjalanan.", style: TextStyle(fontSize: 13, color: Colors.grey)),
            ),
          )
        else
          ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              if (trip.startTime != null && trip.endTime != null) {
                final String formattedStartTime = DateFormat('dd/MM HH:mm', 'id_ID').format(trip.startTime!);
                final String formattedEndTime = DateFormat('HH:mm', 'id_ID').format(trip.endTime!);
                tripTimeInfo = "$formattedStartTime - $formattedEndTime";
              } else if (trip.endTime != null) {
                tripTimeInfo = "Dicatat: ${DateFormat('dd/MM/yyyy HH:mm', 'id_ID').format(trip.endTime!)}";
              }

              String locationDisplay = trip.startAddress?.isNotEmpty == true
                  ? trip.startAddress!
                  : (trip.startLatitude != null ? "Dari: Koordinat" : "Lokasi tidak diketahui");

              if (trip.startAddress?.isNotEmpty == true && trip.endAddress?.isNotEmpty == true && trip.endAddress != trip.startAddress) {
              ...
              return Card(
                ...
                child: ListTile(
                  leading: Icon(Icons.route_outlined, color: Theme.of(context).primaryColor, size: 28),
                  title: Text(
                    locationDisplay,
      ...
      children: [
        _buildNavigationButton(
          ...
          onTap: () {
            ...
            } else { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Data kendaraan tidak tersedia.'))); }
          },
        ),
        _buildNavigationButton(
      ...
      child: Card(
        child: Column(
            ...
            children: [
                ...
                Text(label, textAlign: TextAlign.center, style: const TextStyle(fontSize: 10.5, fontWeight: FontWeight.w500))
            ]
        ),
      ),
    );
